OBJECT Page 9229 Res. Avail. (Service) Matrix
{
  ...
  PROPERTIES
  {
    CaptionML=[DAN=Matrix for ressourcedisponering (service);
               ENU=Res. Availability (Service) Matrix];
    InsertAllowed=No;
    DeleteAllowed=No;
    LinksAllowed=No;
    ...
    ActionList=ACTIONS
    {
      ...
      { 23      ;1   ;ActionGroup;
                      ...
                      Image=Action }
      { 29      ;2   ;Action    ;
                      CaptionML=[DAN=&Alloker;
                                 ENU=&Allocate];
                      ToolTipML=[DAN=Angiver de ressourcer, der kan tildeles.;
                                 ENU=Specifies the resources available to be allocated.];
                      ApplicationArea=#Service;
                      Image=Allocate;
                      OnAction=BEGIN
  ...
  CONTROLS
  {
    ...
    { 15  ;2   ;Field     ;
                Name=Skills;
                CaptionML=[DAN=Ressource kvalificeret til;
                           ENU=Resource Skilled For];
                ToolTipML=[DAN=Angiver, om ressourcekvalifikationerne skal vises for alle serviceartikler i serviceordren eller kun for den valgte serviceartikel.;
                           ENU=Specifies whether the resource skills should be shown for all service items in the service order, or for the selected service item only.];
                OptionCaptionML=[DAN=Valgt serviceartikel,Alle serviceartikler i ordre;
    ...
    { 9   ;2   ;Field     ;
                Name=SelectedDay;
                CaptionML=[DAN=Valgt dato;
                           ENU=Selected Day];
                ToolTipML=[DAN=Angiver den dato, du vëlger for ressourcegruppetilgëngeligheden.;
                           ENU=Specifies the date you select for the resource group availability.];
                ApplicationArea=#Service;
    ...
    { 13  ;2   ;Field     ;
                Name=Qtytoallocate;
                CaptionML=[DAN=Antal til fordeling;
                           ENU=Qty. To Allocate];
                ToolTipML=[DAN=Angiver antallet af timer, der skal allokeres til den valgte ressource.;
                           ENU=Specifies the amount of hours that should be allocated to the selected resource.];
                ApplicationArea=#Service;
    ...
    { 24  ;2   ;Field     ;
                ...
                ToolTipML=[DAN=Angiver nummeret pÜ den ressource, som debitoren foretrëkker ved servicen pÜ serviceartiklen.;
                           ENU=Specifies the number of the resource that the customer prefers for servicing of the service item.];
                ApplicationArea=#Service;
    ...
    { 26  ;2   ;Field     ;
                ...
                CaptionML=[DAN=Kvalificeret;
                           ENU=Skilled];
                ToolTipML=[DAN=Angiver, at ressourcen er tilstrëkkeligt kvalificeret til at udfõre servicen pÜ den serviceartikel, du allokerer, eller alle serviceartikler i serviceordren.;
                           ENU=Specifies that the resource is sufficiently skilled, to carry out the service on the service item that you are allocating or all the service items in the service order.];
                ApplicationArea=#Service;
    ...
    { 28  ;2   ;Field     ;
                ...
                ToolTipML=[DAN=Angiver, at ressourcen (f.eks. en tekniker) er tildelt til samme servicezone som en angivet debitor.;
  ...
  {
    VAR
      Text000@1084 : TextConst 'DAN=Du kan ikke allokere en ressource, nÜr den valgte periode er %1.;ENU=You cannot allocate a resource when selected period is %1.';
      ...
      Text001@1103 : TextConst 'DAN=Dag,Uge,MÜned,Kvartal,èr,Regnskabsperiode;ENU=Day,Week,Month,Quarter,Year,Accounting Period';
    ...
    [Internal]
    PROCEDURE SetData@1109(DocumentType@1003 : Integer;DocumentNo@1000 : Code[20];ServItemLineNo@1001 : Integer;EntryNo@1002 : Integer;NewMatrixColumnCaptions@1004 : ARRAY [32] OF Text[100];VAR NewMatrixDateFilters@1006 : ARRAY [32] OF Record 2000000007;Period@1007 : 'Day,Week,Month,Quarter,Year,Accounting Period');
    BEGIN
