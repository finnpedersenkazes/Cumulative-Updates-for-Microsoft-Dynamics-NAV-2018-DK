OBJECT Codeunit 5601 FA Insert G/L Account
{
  ...
  {
    VAR
      Text000@1000 : TextConst 'DAN=m† ikke v‘re mere end 100;ENU=must not be more than 100';
      Text001@1001 : TextConst 'DAN=Der er ikke linjer nok til at inds‘tte modkontiene.;ENU=There is not enough space to insert the balance accounts.';
    ...
    [Integration]
    LOCAL PROCEDURE OnGetBalAccAfterSaveGenJnlLineFields@23(VAR ToGenJnlLine@1000 : Record 81;FromGenJnlLine@1001 : Record 81);
    BEGIN
    END;
