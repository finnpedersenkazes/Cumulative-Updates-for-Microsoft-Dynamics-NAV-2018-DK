OBJECT Codeunit 5815 Undo Sales Shipment Line
{
  ...
  CODE
  {
    VAR
      ...
      Text000@1003 : TextConst 'DAN=Er du sikker p†, at du vil annullere de valgte leveringslinjer?;ENU=Do you really want to undo the selected Shipment lines?';
      Text001@1009 : TextConst 'DAN=Annuller antalsbogf›ring...;ENU=Undo quantity posting...';
      Text002@1002 : TextConst 'DAN=Der er ikke plads til at inds‘tte rettelseslinjer.;ENU=There is not enough space to insert correction lines.';
      ...
      Text003@1006 : TextConst 'DAN=Kontrollerer linjer...;ENU=Checking lines...';
      Text004@1016 : TextConst 'DAN=Nogle leveringslinjer kan indeholde ubrugte serviceartikler. Skal de slettes?;ENU=Some shipment lines may have unused service items. Do you want to delete them?';
      NextLineNo@1014 : Integer;
      Text005@1010 : TextConst 'DAN=Denne leverance er allerede blevet faktureret. Annuller leverance kan kun bruges p† leverancer, der er blevet bogf›rt, men som ikke er blevet faktureret.;ENU=This shipment has already been invoiced. Undo Shipment can be applied only to posted, but not invoiced shipments.';
      Text006@1011 : TextConst 'DAN=Annuller leverance kan kun anvendes p† linjer af typen Artikel. V‘lg en linje af typen Artikel, og gentag proceduren.;ENU=Undo Shipment can be performed only for lines of type Item. Please select a line of the Item type and repeat the procedure.';
      Text055@1019 : TextConst 'DAN=#1#################################\\Kontrollerer fortryd montage #2###########.;ENU=#1#################################\\Checking Undo Assembly #2###########.';
      Text056@1018 : TextConst 'DAN=#1#################################\\Bogf›rer fortryd montage #2###########.;ENU=#1#################################\\Posting Undo Assembly #2###########.';
      Text057@1012 : TextConst 'DAN=#1#################################\\Afslutter fortryd montage #2###########.;ENU=#1#################################\\Finalizing Undo Assembly #2###########.';
      Text059@1021 : TextConst '@@@="%1 = SalesShipmentLine.""Document No."". %2 = SalesShipmentLine.FIELDCAPTION(""Line No.""). %3 = SalesShipmentLine.""Line No."". This is used in a progress window.";DAN=%1 %2 %3;ENU=%1 %2 %3';
    ...
    [Integration]
    LOCAL PROCEDURE OnAfterUpdateSalesLine@18(VAR SalesLine@1001 : Record 37;SalesShptLine@1000 : Record 111);
    BEGIN
    END;
