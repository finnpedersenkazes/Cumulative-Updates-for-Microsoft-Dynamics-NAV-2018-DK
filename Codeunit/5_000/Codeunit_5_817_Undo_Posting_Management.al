OBJECT Codeunit 5817 Undo Posting Management
{
  ...
  {
    VAR
      Text001@1001 : TextConst 'DAN=Du kan ikke annullere linjen %1, fordi der ikke er nok indhold i modtagelsesplaceringerne.;ENU=You cannot undo line %1 because there is not sufficient content in the receiving bins.';
      Text002@1015 : TextConst 'DAN=Du kan ikke annullere linjen %1, fordi der allerede er oprettet l‘g-p†-lager-linjer.;ENU=You cannot undo line %1 because warehouse put-away lines have already been created.';
      Text003@1018 : TextConst 'DAN=Du kan ikke annullere linjen %1, fordi der allerede er bogf›rt lageraktivitetslinjer.;ENU=You cannot undo line %1 because warehouse activity lines have already been posted.';
      Text004@1019 : TextConst 'DAN=Du skal slette de relaterede %1, f›r du kan annullere linjen %2.;ENU=You must delete the related %1 before you undo line %2.';
      Text005@1020 : TextConst 'DAN=Du kan ikke annullere linjen %1, fordi der allerede er oprettet lagermodtagelseslinjer.;ENU=You cannot undo line %1 because warehouse receipt lines have already been created.';
      Text006@1021 : TextConst 'DAN=Du kan ikke annullere linjen %1, fordi der allerede er oprettet lagerleverancelinjer.;ENU=You cannot undo line %1 because warehouse shipment lines have already been created.';
      Text007@1022 : TextConst 'DAN=Varerne er plukket. Hvis du fortryder linjen %1, forbliver varerne i afsendelsesomr†det, indtil de bliver lagt p† lager.\Vil du stadig fortryde leveringen?;ENU=The items have been picked. If you undo line %1, the items will remain in the shipping area until you put them away.\Do you still want to undo the shipment?';
      Text008@1023 : TextConst 'DAN=Du kan ikke annullere linjen %1, fordi der allerede er bogf›rt lagermodtagelseslinjer.;ENU=You cannot undo line %1 because warehouse receipt lines have already been posted.';
      Text009@1024 : TextConst 'DAN=Du kan ikke annullere linjen %1, fordi der allerede er bogf›rt l‘g-p†-lager-linjer.;ENU=You cannot undo line %1 because warehouse put-away lines have already been posted.';
      Text010@1025 : TextConst 'DAN=Du kan ikke annullere linjen %1, fordi der allerede er bogf›rt pluklinjer.;ENU=You cannot undo line %1 because inventory pick lines have already been posted.';
      Text011@1026 : TextConst 'DAN=Du kan ikke annullere linjen %1, fordi der er knyttet et varegebyr til den i %2 dok.nr. %3 linje %4.;ENU=You cannot undo line %1 because there is an item charge assigned to it on %2 Doc No. %3 Line %4.';
      Text012@1002 : TextConst 'DAN=Du kan ikke annullere linjen %1, fordi der allerede er faktureret et varegebyr.;ENU=You cannot undo line %1 because an item charge has already been invoiced.';
      Text013@1003 : TextConst 'DAN=Der mangler vareposter for linje %1.;ENU=Item ledger entries are missing for line %1.';
      Text014@1004 : TextConst 'DAN=Du kan ikke annullere linjen %1, fordi der allerede er bogf›rt en v‘rdiregulering.;ENU=You cannot undo line %1, because a revaluation has already been posted.';
      Text015@1005 : TextConst 'DAN=Du kan ikke fortryde bogf›ring af varen %1 med varianten ''%2'' og enheden %3, da den ikke findes p† lokation %4, placeringskode %5. Det n›dvendige antal er %6. Det disponible antal er %7.;ENU=You cannot undo posting of item %1 with variant ''%2'' and unit of measure %3 because it is not available at location %4, bin code %5. The required quantity is %6. The available quantity is %7.';
    ...
    [External]
    PROCEDURE PostItemJnlLineAppliedToList@22(ItemJnlLine@1002 : Record 83;VAR TempApplyToEntryList@1000 : TEMPORARY Record 32;UndoQty@1009 : Decimal;UndoQtyBase@1010 : Decimal;VAR TempItemLedgEntry@1011 : TEMPORARY Record 32;VAR TempItemEntryRelation@1008 : TEMPORARY Record 6507);
    VAR
    ...
    VAR
      ...
      PostJobConsumptionBeforePurch@1004 : Boolean;
    BEGIN
      CLEAR(ItemJnlLine2);
    ...
    LOCAL PROCEDURE SetItemJnlLineAppliesToEntry@38(VAR ItemJnlLine@1000 : Record 83;AppliesToEntry@1002 : Integer);
    VAR
      Item@1001 : Record 27;
    BEGIN
    ...
    [Integration]
    LOCAL PROCEDURE OnAfterUpdateSalesLine@44(VAR SalesLine@1000 : Record 37);
    BEGIN
    END;

    [Integration]
    LOCAL PROCEDURE OnAfterUpdatePurchline@35(VAR PurchLine@1000 : Record 39);
    BEGIN
    END;

    [Integration]
    LOCAL PROCEDURE OnAfterUpdateServLine@36(VAR ServLine@1000 : Record 5902);
    BEGIN
    END;
