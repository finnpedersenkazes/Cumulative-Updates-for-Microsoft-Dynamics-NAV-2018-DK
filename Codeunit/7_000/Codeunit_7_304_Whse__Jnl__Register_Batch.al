OBJECT Codeunit 7304 Whse. Jnl.-Register Batch
{
  ...
  CODE
  {
    VAR
      ...
      Text001@1001 : TextConst 'DAN=Kladdenavn            #1##########\\;ENU=Journal Batch Name    #1##########\\';
      Text002@1002 : TextConst 'DAN=Linjerne kontrolleres #2######\;ENU=Checking lines        #2######\';
      Text003@1005 : TextConst 'DAN=Linjerne registreres  #3###### @4@@@@@@@@@@@@@;ENU=Registering lines     #3###### @4@@@@@@@@@@@@@';
      Text004@1006 : TextConst 'DAN=Der kan maksimalt bruges %1 registreringsnummerserier i hver kladde.;ENU=A maximum of %1 registering number series can be used in each journal.';
      Text007@1009 : TextConst '@@@="One or more reservation entries exist for the item with Item No. = 1000, Location Code = BLUE, Variant Code = NEW which may be disrupted if you post this negative adjustment. Do you want to continue?";DAN="Der findes en eller flere reservationsposter til varen %1 = %2, %3 = %4, %5 = %6, som kan blive afbrudt, hvis du bogf›rer denne nedregulering. Vil du forts‘tte?";ENU="One or more reservation entries exist for the item with %1 = %2, %3 = %4, %5 = %6 which may be disrupted if you post this negative adjustment. Do you want to continue?"';
      ...
      Text005@1003 : TextConst 'DAN=Varesporingslinjer, der er angivet for kildelinjen, skal indeholde det samme antal, som du har angivet.;ENU=Item tracking lines defined for the source line must account for the same quantity as you have entered.';
      Text006@1004 : TextConst 'DAN=Varesporingslinjerne stemmer ikke overens med placeringsindholdet.;ENU=Item tracking lines do not match the bin content.';
    ...
    [Integration]
    LOCAL PROCEDURE OnCheckWhseJnlLine@15(VAR WhseJnlLine@1000 : Record 7311);
    BEGIN
    END;

    [Integration]
    LOCAL PROCEDURE OnBeforeInsertTempHandlingSpecs@16(VAR WhseJnlLine@1000 : Record 7311;VAR WhseItemTrackingLine@1001 : Record 6550);
    BEGIN
    END;

    [Integration]
    LOCAL PROCEDURE OnAfterCollectTrackingInformation@17(VAR WhseJnlLine@1000 : Record 7311);
    BEGIN
    END;

    [Integration]
    LOCAL PROCEDURE OnAfterItemJnlPostLine@19(VAR WhseJnlLine@1000 : Record 7311);
    BEGIN
    END;
