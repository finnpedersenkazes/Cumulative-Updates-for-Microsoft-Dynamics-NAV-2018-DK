OBJECT Codeunit 1002 Job Create-Invoice
{
  ...
  CODE
  {
    VAR
      ...
      Text000@1008 : TextConst 'DAN=Linjerne er overf›rt til en faktura.;ENU=The lines were successfully transferred to an invoice.';
      Text001@1009 : TextConst 'DAN=Linjerne blev ikke overf›rt til en faktura.;ENU=The lines were not transferred to an invoice.';
      Text002@1010 : TextConst 'DAN=Der var ingen %1 med en %2 st›rre end 0. Ingen linjer blev overf›rt.;ENU=There was no %1 with a %2 larger than 0. No lines were transferred.';
      Text003@1011 : TextConst 'DAN=%1 m† ikke v‘re lavere end %2 og m† ikke overskride %3.;ENU=%1 may not be lower than %2 and may not exceed %3.';
      Text004@1015 : TextConst 'DAN=Du skal angive et fakturanr. eller en ny faktura.;ENU=You must specify Invoice No. or New Invoice.';
      Text005@1014 : TextConst 'DAN=Du skal angive et kreditnotanr. eller en ny kreditnota.;ENU=You must specify Credit Memo No. or New Invoice.';
      Text007@1012 : TextConst 'DAN=Du skal indtaste %1.;ENU=You must specify %1.';
      TransferExtendedText@1016 : Codeunit 378;
      JobInvCurrency@1002 : Boolean;
      UpdateExchangeRates@1019 : Boolean;
      Text008@1018 : TextConst 'DAN=Linjerne er overf›rt til en kreditnota.;ENU=The lines were successfully transferred to a credit memo.';
      Text009@1003 : TextConst 'DAN=De valgte planl‘gningslinjer skal have samme %1.;ENU=The selected planning lines must have the same %1.';
      Text010@1004 : TextConst 'DAN=Valutadatoerne p† alle planl‘gningslinjer vil blive opdateret p† grundlag af bogf›ringsdatoen, fordi der er en forskel i valutakurserne. Genberegning vil v‘re baseret p† ops‘tningen af Kursberegning for v‘rdierne for Omkostning og Pris for sagen. Vil du forts‘tte?;ENU=The currency dates on all planning lines will be updated based on the invoice posting date because there is a difference in currency exchange rates. Recalculations will be based on the Exch. Calculation setup for the Cost and Price values for the job. Do you want to continue?';
      Text011@1007 : TextConst 'DAN=Valutakursen for alle planl‘gningslinjer opdateres p† baggrund af valutakursen p† salgsfakturaen. Vil du forts‘tte?;ENU=The currency exchange rate on all planning lines will be updated based on the exchange rate on the sales invoice. Do you want to continue?';
      Text012@1013 : TextConst '@@@=The Sales Invoice Header 103001 does not exist in the system anymore. A printed copy of the document was created before deletion.;DAN=%1 %2 findes ikke l‘ngere. Der blev oprettet en udskrevet kopi af dokumentet, f›r dokumentet blev slettet.;ENU=The %1 %2 does not exist anymore. A printed copy of the document was created before the document was deleted.';
    ...
    [Internal]
    PROCEDURE CreateSalesInvoiceJobTask@7(VAR JobTask2@1000 : Record 1001;PostingDate@1006 : Date;InvoicePerTask@1007 : Boolean;VAR NoOfInvoices@1008 : Integer;VAR OldJobNo@1004 : Code[20];VAR OldJobTaskNo@1009 : Code[20];LastJobTask@1010 : Boolean);
    VAR
      Cust@1003 : Record 18;
      Job@1002 : Record 167;
    ...
    LOCAL PROCEDURE CreateNewInvoice@11(VAR JobTask@1003 : Record 1001;InvoicePerTask@1005 : Boolean;VAR OldJobNo@1000 : Code[20];VAR OldJobTaskNo@1001 : Code[20];LastJobTask@1002 : Boolean) : Boolean;
    VAR
    ...
    [Integration]
    LOCAL PROCEDURE OnBeforeInsertSalesHeader@19(VAR SalesHeader@1001 : Record 36;Job@1000 : Record 167);
    BEGIN
    END;

    [Integration]
    LOCAL PROCEDURE OnBeforeModifySalesHeader@24(VAR SalesHeader@1000 : Record 36;Job@1001 : Record 167);
    BEGIN
    END;

    [Integration]
    LOCAL PROCEDURE OnBeforeInsertSalesLine@22(VAR SalesLine@1003 : Record 37;SalesHeader@1002 : Record 36;Job@1001 : Record 167;JobPlanningLine@1000 : Record 1003);
    BEGIN
    END;

    [Integration]
    LOCAL PROCEDURE OnBeforeModifySalesLine@23(VAR SalesLine@1000 : Record 37;SalesHeader@1001 : Record 36;Job@1002 : Record 167;JobPlanningLine@1003 : Record 1003);
    BEGIN
    END;
