OBJECT Report 5198 Add Contacts
{
  ...
  REQUESTPAGE
  {
    ...
    CONTROLS
    {
      ...
      { 3   ;2   ;Field     ;
                  CaptionML=[DAN=Tillad eksis. kontakter;
                             ENU=Allow Existing Contacts];
                  ToolTipML=[DAN=Angiver, om eksisterende kontakter er medtaget i m†lgruppen.;
                             ENU=Specifies if existing contacts are included in the segment.];
                  ApplicationArea=#RelationshipMgmt;
                  SourceExpr=AllowExistingContact }

      { 1   ;2   ;Field     ;
                  CaptionML=[DAN=Udvid virksomheder;
                             ENU=Expand Companies];
                  ToolTipML=[DAN=Angiver, om m†lgruppen skal omfatte alle personkontakter, der arbejder for den virksomhed, som du vil f›je til m†lgruppen.;
      ...
      { 7   ;2   ;Field     ;
                  CaptionML=[DAN=Ignorer udelukkelse;
                             ENU=Ignore Exclusion];
                  ToolTipML=[DAN=Angiver, om kontakter ikke skal medtages, for hvilke feltet Udeluk fra m†lgruppe er blevet valgt p† kontaktkortet.;
  ...
  {
    VAR
      Text000@1000 : TextConst 'DAN=Inds‘tter kontakt  @1@@@@@@@@@@@@@;ENU=Inserting contacts @1@@@@@@@@@@@@@';
