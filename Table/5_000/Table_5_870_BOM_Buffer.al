OBJECT Table 5870 BOM Buffer
{
  ...
  FIELDS
  {
    ...
    { 2   ;   ;Type                ;Option        ;DataClassification=SystemMetadata;
                                                   CaptionML=[DAN=Type;
                                                              ENU=Type];
                                                   OptionCaptionML=[DAN=,Vare,Prod.ressource,Arbejdscenter,Ressource;
                                                                    ENU=,Item,Machine Center,Work Center,Resource];
    ...
    { 42  ;   ;Rolled-up Lead-Time Offset;Integer ;DataClassification=SystemMetadata;
                                                   CaptionML=[DAN=Akkumuleret genneml›bstid;
                                                              ENU=Rolled-up Lead-Time Offset] }
    ...
    { 56  ;   ;Scrap Qty. per Parent;Decimal      ;DataClassification=SystemMetadata;
                                                   CaptionML=[DAN=Spildantal pr. overordnet;
                                                              ENU=Scrap Qty. per Parent];
                                                   DecimalPlaces=0:5 }
    { 57  ;   ;Scrap Qty. per Top Item;Decimal    ;DataClassification=SystemMetadata;
                                                   CaptionML=[DAN=Spildantal pr. topvare;
                                                              ENU=Scrap Qty. per Top Item];
                                                   DecimalPlaces=0:5 }
    ...
    { 66  ;   ;Single-Level Scrap Cost;Decimal    ;DataClassification=SystemMetadata;
                                                   CaptionML=[DAN=Spildomkostninger p† enkeltniveau;
                                                              ENU=Single-Level Scrap Cost];
                                                   DecimalPlaces=2:5;
                                                   BlankZero=Yes }
    { 71  ;   ;Rolled-up Material Cost;Decimal    ;DataClassification=SystemMetadata;
                                                   CaptionML=[DAN=Akkum. materialekostpris;
    ...
    { 72  ;   ;Rolled-up Capacity Cost;Decimal    ;DataClassification=SystemMetadata;
                                                   CaptionML=[DAN=Akkum. kapacitetskostpris;
    ...
    { 74  ;   ;Rolled-up Capacity Ovhd. Cost;Decimal;
                                                   DataClassification=SystemMetadata;
                                                   CaptionML=[DAN=Akkumulerede indirekte kapacitetsomkostninger;
                                                              ENU=Rolled-up Capacity Ovhd. Cost];
                                                   BlankZero=Yes;
                                                   Editable=No;
                                                   AutoFormatType=2 }
    { 75  ;   ;Rolled-up Mfg. Ovhd Cost;Decimal   ;DataClassification=SystemMetadata;
                                                   CaptionML=[DAN=Ind. prod.kostpris (akkum.);
                                                              ENU=Rolled-up Mfg. Ovhd Cost];
                                                   BlankZero=Yes;
                                                   Editable=No;
                                                   AutoFormatType=2 }
    { 76  ;   ;Rolled-up Scrap Cost;Decimal       ;DataClassification=SystemMetadata;
                                                   CaptionML=[DAN=Akkumulerede spildomkostninger;
                                                              ENU=Rolled-up Scrap Cost];
                                                   DecimalPlaces=2:5;
                                                   BlankZero=Yes }
    ...
    { 82  ;   ;BOM Unit of Measure Code;Code10    ;TableRelation=IF (Type=CONST(Item)) "Item Unit of Measure".Code WHERE (Item No.=FIELD(No.))
                                                                 ELSE IF (Type=CONST(Resource)) "Resource Unit of Measure".Code WHERE (Resource No.=FIELD(No.));
                                                   DataClassification=SystemMetadata;
                                                   CaptionML=[DAN=M†leenhedskode for stykliste;
                                                              ENU=BOM Unit of Measure Code] }
    { 83  ;   ;Qty. per BOM Line   ;Decimal       ;DataClassification=SystemMetadata;
                                                   CaptionML=[DAN=Ant. pr. styklistelinje;
                                                              ENU=Qty. per BOM Line];
                                                   DecimalPlaces=0:5 }
  }
  KEYS
  {
  ...
  CODE
  {
    VAR
      ...
      Text001@1008 : TextConst 'DAN=Laveste-niveau-koden for vare %1 er ikke blevet beregnet.;ENU=The Low-level Code for Item %1 has not been calculated.';
      Text002@1007 : TextConst 'DAN=Antal pr. felt i styklisten for vare %1 er ikke blevet angivet.;ENU=The Quantity per. field in the BOM for Item %1 has not been set.';
      Text003@1006 : TextConst 'DAN=Ruten %1 er ikke blevet certificeret.;ENU=Routing %1 has not been certified.';
      Text004@1005 : TextConst 'DAN=Produktionsstyklisten %1 er ikke blevet certificeret.;ENU=Production BOM %1 has not been certified.';
      Text005@1004 : TextConst 'DAN=Varen %1 har ikke en stykliste. Feltet Genbestillingssystemet skal derfor v‘re K›b.;ENU=Item %1 is not a BOM. Therefore, the Replenishment System field must be set to Purchase.';
      Text006@1003 : TextConst 'DAN=Genbestillingssystemet for vare %1 er Montage, men varen har ikke en montagestykliste. Kontroll‚r, at dette er korrekt.;ENU=Replenishment System for Item %1 is Assembly, but the item is not an assembly BOM. Verify that this is correct.';
      Text007@1002 : TextConst 'DAN=Genbestillingssystemet for vare %1 er Prod.ordre, men varen har ikke en produktionsstykliste. Kontroll‚r, at dette er korrekt;ENU=Replenishment System for Item %1 is Prod. Order, but the item does not have a production BOM. Verify that this is correct.';
      Text008@1001 : TextConst 'DAN=Varen %1 er en stykliste, men genbestillingssystemet er ikke Montage eller Prod.ordre. Kontroll‚r, at v‘rdien er korrekt.;ENU=Item %1 is a BOM, but the Replenishment System field is not set to Assembly or Prod. Order. Verify that the value is correct.';
