OBJECT Table 5900 Service Header
{
  ...
  FIELDS
  {
    ...
    { 130 ;   ;Release Status      ;Option        ;CaptionML=[DAN=Frigivelsesstatus;
                                                              ENU=Release Status];
                                                   OptionCaptionML=[DAN=èben,Frigivet til afsendelse;
    ...
    { 5931;   ;Finishing Date      ;Date          ;OnValidate=BEGIN
                                                   ...
                                                   CaptionML=[DAN=Fërdiggõrelsesdato;
                                                              ENU=Finishing Date] }
    { 5932;   ;Finishing Time      ;Time          ;OnValidate=BEGIN
                                                   ...
                                                   CaptionML=[DAN=Fërdiggõrelsestidspunkt;
                                                              ENU=Finishing Time] }
  ...
  CODE
  {
    VAR
      ...
      Text001@1001 : TextConst 'DAN=Hvis du ëndrer %1 i servicehovedet %2, opdateres de eksisterende servicelinjer ikke.\Du skal opdatere de eksisterende servicelinjer manuelt.;ENU=Changing %1 in service header %2 will not update the existing service lines.\You must update the existing service lines manually.';
      Text003@1003 : TextConst '@@@="%1=Customer number field caption;%2=Document type;%3=Number field caption;%4=Number;%5=Contract number field caption;%6=Contract number; ";DAN=Du kan ikke ëndre %1, fordi %2 %3 %4 er tilknyttet en %5 %6.;ENU=You cannot change the %1 because the %2 %3 %4 is associated with a %5 %6.';
      Text004@1004 : TextConst 'DAN=Hvis du ëndrer %1, slettes den eksisterende serviceartikellinje og den eksisterende servicelinje.\Vil du ëndre %1?;ENU=When you change the %1 the existing Service item line and service line will be deleted.\Do you want to change the %1?';
      Text005@1005 : TextConst 'DAN=Vil du ëndre %1?;ENU=Do you want to change the %1?';
      Text007@1007 : TextConst 'DAN=%1 kan ikke vëre stõrre end %2.;ENU=%1 cannot be greater than %2.';
      ...
      Text011@1011 : TextConst 'DAN=%1 kan ikke vëre stõrre end %2 i tabellen %3.;ENU=%1 cannot be greater than %2 in the %3 table.';
      Text012@1012 : TextConst 'DAN=Hvis du ëndrer %1, vil de eksisterende servicelinjer blive slettet, og der vil blive oprettet nye servicelinjer pÜ baggrund af de nye oplysninger i hovedet.\Vil du ëndre %1?;ENU=If you change %1, the existing service lines will be deleted and the program will create new service lines based on the new information on the header.\Do you want to change the %1?';
      Text013@1089 : TextConst 'DAN=Hvis du sletter dette dokument, opstÜr der et hul i nummerserien for bogfõrte kreditnotaer. Der oprettes en tom bogfõrt kreditnota %1 for at udfylde hullet i nummerserien.\\Vil du fortsëtte?;ENU=Deleting this document will cause a gap in the number series for posted credit memos. An empty posted credit memo %1 will be created to fill this gap in the number series.\\Do you want to continue?';
      Text015@1015 : TextConst 'DAN=Vil du opdatere valutakursen?;ENU=Do you want to update the exchange rate?';
      Text016@1016 : TextConst 'DAN=Du har ëndret %1.\Vil du opdatere servicelinjerne?;ENU=You have modified %1.\Do you want to update the service lines?';
      ...
      Text019@1019 : TextConst 'DAN=Du har ëndret %1 pÜ servicehovedet, men vërdien er ikke blevet ëndret pÜ de eksisterende servicelinjer.\índringen kan pÜvirke den anvendte valutakurs i prisberegningen pÜ servicelinjerne.;ENU=You have changed %1 on the service header, but it has not been changed on the existing service lines.\The change may affect the exchange rate used in the price calculation of the service lines.';
      Text021@1021 : TextConst 'DAN=Du har ëndret %1 pÜ %2, men vërdien er ikke blevet ëndret pÜ de eksisterende servicelinjer.\Du skal opdatere de eksisterende servicelinjer manuelt.;ENU=You have changed %1 on the %2, but it has not been changed on the existing service lines.\You must update the existing service lines manually.';
      ...
      Text024@1002 : TextConst 'DAN=%1 kan ikke vëre stõrre end den mindste %1 pÜ\serviceartikellinjerne.;ENU=The %1 cannot be greater than the minimum %1 of the\ Service Item Lines.';
      Text025@1050 : TextConst 'DAN=%1 kan ikke vëre mindre end den stõrste %1 pÜ de relaterede\ serviceartikellinjer.;ENU=The %1 cannot be less than the maximum %1 of the related\ Service Item Lines.';
      Text026@1051 : TextConst 'DAN=%1 mÜ ikke vëre tidligere end %2.;ENU=%1 cannot be earlier than the %2.';
      Text027@1052 : TextConst 'DAN=%1 mÜ ikke vëre stõrre end den mindste %2 pÜ de relaterede\ serviceartikellinjer.;ENU=The %1 cannot be greater than the minimum %2 of the related\ Service Item Lines.';
      ValidatingFromLines@1053 : Boolean;
      LinesExist@1054 : Boolean;
      Text028@1057 : TextConst 'DAN=%1 kan ikke ëndres, fordi %2 findes.;ENU=You cannot change the %1 because %2 exists.';
      Text029@1056 : TextConst 'DAN=Feltet %1 pÜ %2 opdateres, hvis du ëndrer %3 manuelt.\Vil du fortsëtte?;ENU=The %1 field on the %2 will be updated if you change %3 manually.\Do you want to continue?';
      Text031@1059 : TextConst '@@@="%1=Status field caption;%2=Status format;%3=table caption;%4=Number;%5=ServItemLine repair status code field caption;%6=ServItemLine repair status code;%7=ServItemLine table caption;%8=ServItemLine line number;";DAN=Du kan ikke ëndre %1 til %2 i %3 %4.\\%5 %6 i linjen %7 %8 forhindrer dette.;ENU=You cannot change %1 to %2 in %3 %4.\\%5 %6 in %7 %8 line is preventing it.';
      ...
      Text047@1100 : TextConst 'DAN=Du kan ikke ëndre %1, fordi der er reservation, varesporing eller ordresporing pÜ salgsordren.;ENU=You cannot change %1 because reservation, item tracking, or order tracking exists on the sales order.';
      ...
      Text053@1093 : TextConst 'DAN=Hvis du sletter dette dokument, opstÜr der et hul i nummerserien for leverancer. Der oprettes en tom leverance %1 for at udfylde hullet i nummerserien.\\Vil du fortsëtte?;ENU=Deleting this document will cause a gap in the number series for shipments. An empty shipment %1 will be created to fill this gap in the number series.\\Do you want to continue?';
      Text054@1091 : TextConst 'DAN=Hvis du sletter dette dokument, opstÜr der et hul i nummerserien for bogfõrte fakturaer. Der oprettes en tom bogfõrt faktura %1 for at udfylde hullet i nummerserien.\\Vil du fortsëtte?;ENU=Deleting this document will cause a gap in the number series for posted invoices. An empty posted invoice %1 will be created to fill this gap in the number series.\\Do you want to continue?';
      Text055@1079 : TextConst 'DAN=Du har redigeret feltet %1. Bemërk, at genberegningen af moms kan resultere i õredifferencer, sÜ du skal kontrollere belõbene efter genberegningen. Vil du opdatere feltet %2 pÜ linjerne, sÜ den nye vërdi for %1 afspejles?;ENU=You have modified the %1 field. Note that the recalculation of VAT may cause penny differences, so you must check the amounts afterwards. Do you want to update the %2 field on the lines to reflect the new value of %1?';
      Text057@1103 : TextConst 'DAN=NÜr du ëndrer %1, slettes den eksisterende servicelinje.\Vil du ëndre %1?;ENU=When you change the %1 the existing service line will be deleted.\Do you want to change the %1?';
      Text058@1088 : TextConst '@@@="%1=Currency code field caption;%2=Document type;%3=Number;%4=Contract number;";DAN=Du kan ikke ëndre %1, fordi %2 %3 er tilknyttet kontrakt %4.;ENU=You cannot change %1 because %2 %3 is linked to Contract %4.';
      ...
      Text061@1099 : TextConst 'DAN=Du har muligvis ëndret en dimension.\\Vil du opdatere linjerne?;ENU=You may have changed a dimension.\\Do you want to update the lines?';
      Text062@1087 : TextConst 'DAN=Der findes et Übent lagerpluk for %1, og fordi %2 er angivet til %3.\\Du skal fõrst bogfõre eller slette lagerplukket eller ëndre %2 til Delvis.;ENU=An open inventory pick exists for the %1 and because %2 is %3.\\You must first post or delete the inventory pick or change %2 to Partial.';
      Text063@1102 : TextConst 'DAN=Der findes en Üben lagerleverance for %1, og %2 er %3.\\Du kan tilfõje varerne som nye linjer i den eksisterende lagerleverance eller ëndre %2 til Delvis.;ENU=An open warehouse shipment exists for the %1 and %2 is %3.\\You must add the item(s) as new line(s) to the existing warehouse shipment or change %2 to Partial.';
      Text064@1105 : TextConst 'DAN=Du kan ikke ëndre %1 til %2 grundet et Übent pluk (lager) pÜ %3.;ENU=You cannot change %1 to %2 because an open inventory pick on the %3.';
      Text065@1106 : TextConst 'DAN=Du kan ikke ëndre %1 til %2, fordi der findes en Üben lagerleverance for %3.;ENU=You cannot change %1  to %2 because an open warehouse shipment exists for the %3.';
      ...
      Text13607@1101100002 : TextConst 'DAN=%1 %2 understõttes ikke i denne version af OIOUBL.;ENU=%1 %2 is not supported in this version of OIOUBL.';
      Text066@1017 : TextConst 'DAN=Du kan ikke ëndre dimensionen, da der er serviceposter forbundet til denne linje.;ENU=You cannot change the dimension because there are service entries connected to this line.';
      PostedDocsToPrintCreatedMsg@1022 : TextConst 'DAN=Et eller flere relaterede bogfõrte dokumenter er genereret under sletningen for at udfylde huller i bogfõringen af nummerserier. Du kan vise eller udskrive dokumenterne fra det respektive dokumentarkiv.;ENU=One or more related posted documents have been generated during deletion to fill gaps in the posting number series. You can view or print the documents from the respective document archive.';
      DocumentNotPostedClosePageQst@1020 : TextConst 'DAN=Dokumentet er ikke bogfõrt.\Er du sikker pÜ, at du vil afslutte?;ENU=The document has not been posted.\Are you sure you want to exit?';
