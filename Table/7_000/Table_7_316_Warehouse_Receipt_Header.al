OBJECT Table 7316 Warehouse Receipt Header
{
  ...
  PROPERTIES
  {
    ...
    CaptionML=[DAN=Lagermodtagelseshoved;
               ENU=Warehouse Receipt Header];
  ...
  FIELDS
  {
    ...
    { 9   ;   ;Bin Code            ;Code20        ;TableRelation=IF (Zone Code=FILTER('')) Bin.Code WHERE (Location Code=FIELD(Location Code))
                                                   ...
                                                   CaptionML=[DAN=Placeringskode;
                                                              ENU=Bin Code] }
    { 10  ;   ;Document Status     ;Option        ;CaptionML=[DAN=Bilagsstatus;
                                                              ENU=Document Status];
    ...
    { 50  ;   ;Create Posted Header;Boolean       ;CaptionML=[DAN=Opret bogf›rt hoved;
                                                              ENU=Create Posted Header] }
    { 51  ;   ;Receiving No.       ;Code20        ;CaptionML=[DAN=K›bslev.nr.;
                                                              ENU=Receiving No.] }
  ...
  CODE
  {
    VAR
      ...
      Text000@1007 : TextConst 'DAN=%1 kan ikke omd›bes.;ENU=You cannot rename a %1.';
      Text001@1006 : TextConst 'DAN=%1 kan ikke ‘ndres, fordi dokumentet indeholder en eller flere linjer.;ENU=You cannot change the %1, because the document has one or more lines.';
      Text002@1005 : TextConst 'DAN=Du skal f›rst oprette brugeren %1 som lagermedarbejder.;ENU=You must first set up user %1 as a warehouse employee.';
      Text003@1008 : TextConst 'DAN=Du kan ikke bruge lokationskode %1.;ENU=You are not allowed to use location code %1.';
      Text005@1014 : TextConst 'DAN=kan ikke v‘re %1 i %2;ENU=must not be the %1 of the %2';
      Text006@1013 : TextConst 'DAN=Du har ‘ndret %1 p† %2, men der er ikke foretaget ‘ndringer p† de eksisterende lagermodtagelseslinjer.\;ENU=You have changed %1 on the %2, but it has not been changed on the existing Warehouse Receipt Lines.\';
      Text007@1012 : TextConst 'DAN=Du skal opdatere de eksisterende lagermodtagelseslinjer manuelt.;ENU=You must update the existing Warehouse Receipt Lines manually.';
      HideValidationDialog@1015 : Boolean;
      Text008@1010 : TextConst 'DAN=Modtagelsen er ikke fuldt modtaget.\Er du sikker p†, at vil slette modtagelsen?;ENU=The Whse. Receipt is not completely received.\Do you really want to delete the Whse. Receipt?';
      Text009@1016 : TextConst 'DAN=Annulleret.;ENU=Cancelled.';
