OBJECT Table 7347 Internal Movement Line
{
  ...
  PROPERTIES
  {
    ...
    CaptionML=[DAN=Linje - intern flytning;
               ENU=Internal Movement Line];
  ...
  CODE
  {
    VAR
      ...
      Text001@1006 : TextConst 'DAN=m† ikke v‘re mere end %1 enheder;ENU=must not be greater than %1 units';
      Text002@1007 : TextConst 'DAN=%1 kan ikke omd›bes.;ENU=You cannot rename a %1.';
      LastLineNo@1012 : Integer;
      ItemTrackingErr@1005 : TextConst '@@@=Item tracking numbers should not be higher than the item quantity.;DAN=Varesporingsnumre, der defineres for varen %1 i %2, er h›jere end vareantallet.\\Juster varesporingsnummeret, og ›g vareantallet, hvis det er relevant.;ENU=Item tracking numbers defined for item %1 in the %2 are higher than the item quantity.\\Adjust the item tracking numbers and then increase the item quantity, if relevant.';
