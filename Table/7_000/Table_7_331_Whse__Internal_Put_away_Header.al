OBJECT Table 7331 Whse. Internal Put-away Header
{
  ...
  PROPERTIES
  {
    ...
    CaptionML=[DAN=Intern lëg-pÜ-lager-hvd (log.);
               ENU=Whse. Internal Put-away Header];
  ...
  FIELDS
  {
    ...
    { 11  ;   ;Document Status     ;Option        ;OnValidate=VAR
                                                   ...
                                                   CaptionML=[DAN=Bilagsstatus;
                                                              ENU=Document Status];
                                                   OptionCaptionML=[DAN=" ,Delvist lagt-pÜ-lager,Fuldt lagt-pÜ-lager";
  ...
  CODE
  {
    VAR
      ...
      Text001@1008 : TextConst 'DAN=kan ikke vëre %1 i %2;ENU=must not be the %1 of the %2';
      Text002@1007 : TextConst 'DAN=Du har ëndret %1 pÜ %2, men der er ikke foretaget ëndringer pÜ de eksisterende interne lëg-pÜ-lager-linjer.\;ENU=You have changed %1 on the %2, but it has not been changed on the existing Warehouse Internal Put-Away Lines.\';
      Text003@1006 : TextConst 'DAN=Du skal opdatere de eksisterende interne lëg-pÜ-lager-linjer manuelt.;ENU=You must update the existing Warehouse Internal Put-Away Lines manually.';
      Text004@1010 : TextConst 'DAN=%1 kan ikke omdõbes.;ENU=You cannot rename a %1.';
      Text005@1012 : TextConst 'DAN=%1 kan ikke ëndres, fordi dokumentet indeholder en eller flere linjer.;ENU=You cannot change the %1, because the document has one or more lines.';
      Text006@1013 : TextConst 'DAN=Du kan ikke bruge %1 %2.;ENU=You are not allowed to use %1 %2.';
    ...
    [External]
    PROCEDURE LookupInternalPutAwayHeader@1(VAR WhseInternalPutAwayHeader@1001 : Record 7331);
    BEGIN
      COMMIT;
      IF USERID <> '' THEN BEGIN
    ...
    [External]
    PROCEDURE LookupLocation@4(VAR WhseInternalPutAwayHeader@1001 : Record 7331);
    VAR
