OBJECT Table 7333 Whse. Internal Pick Header
{
  ...
  PROPERTIES
  {
    ...
    CaptionML=[DAN=Internt plukhoved (logistik);
               ENU=Whse. Internal Pick Header];
  ...
  FIELDS
  {
    ...
    { 34  ;   ;Document Status     ;Option        ;OnValidate=VAR
                                                   ...
                                                   CaptionML=[DAN=Bilagsstatus;
                                                              ENU=Document Status];
                                                   OptionCaptionML=[DAN=" ,Delvist plukket,Fuldt plukket";
                                                                    ENU=" ,Partially Picked,Completely Picked"];
  ...
  CODE
  {
    VAR
      ...
      Text003@1005 : TextConst 'DAN=Du kan ikke bruge %1 %2.;ENU=You are not allowed to use %1 %2.';
      ItemTrackingMgt@1006 : Codeunit 6500;
      HideValidationDialog@1007 : Boolean;
      Text005@1008 : TextConst 'DAN=kan ikke vëre %1 i %2;ENU=must not be the %1 of the %2';
      Text006@1012 : TextConst 'DAN=Du har ëndret %1 pÜ %2, men der er ikke foretaget ëndringer pÜ de eksisterende interne lagerpluklinjer.\;ENU=You have changed %1 on the %2, but it has not been changed on the existing Warehouse Internal Pick Lines.\';
      Text007@1009 : TextConst 'DAN=Du skal opdatere de eksisterende interne lagerpluklinjer manuelt.;ENU=You must update the existing Warehouse Internal Pick Lines manually.';
    ...
    [External]
    PROCEDURE LookupWhseInternalPickHeader@1(VAR WhseInternalPickHeader@1001 : Record 7333);
    BEGIN
      COMMIT;
      IF USERID <> '' THEN BEGIN
    ...
    [External]
    PROCEDURE LookupLocation@4(VAR WhseInternalPickHeader@1001 : Record 7333);
    VAR
