OBJECT Table 14 Location
{
  ...
  FIELDS
  {
    ...
    { 5726;   ;Require Put-away    ;Boolean       ;OnValidate=VAR
                                                   ...
                                                   CaptionML=[DAN=Krëv lëg-pÜ-lager;
    ...
    { 5733;   ;Directed Put-away and Pick;Boolean ;OnValidate=VAR
                                                   ...
                                                   AccessByPermission=TableData 5771=R;
                                                   CaptionML=[DAN=Styret lëg-pÜ-lager og pluk;
    ...
    { 7313;   ;Open Shop Floor Bin Code;Code20    ;TableRelation=Bin.Code WHERE (Location Code=FIELD(Code));
                                                   ...
                                                   CaptionML=[DAN=èben prod.placeringskode;
    ...
    { 7319;   ;Always Create Put-away Line;Boolean;AccessByPermission=TableData 5771=R;
                                                   CaptionML=[DAN=Opret altid lëg-pÜ-lager-linje;
  ...
  CODE
  {
    VAR
      ...
      Text001@1006 : TextConst 'DAN=Du kan ikke slette %1 %2 , fordi der findes Çn eller flere lageraktivitetslinjer til %1.;ENU=You cannot delete the %1 %2, because one or more Warehouse Activity Lines exist for this %1.';
      Text002@1007 : TextConst 'DAN=%1 skal vëre Ja, fordi placeringerne indeholder varer.;ENU=%1 must be Yes, because the bins contain items.';
      Text003@1009 : TextConst 'DAN=Annulleret.;ENU=Cancelled.';
      Text004@1010 : TextConst 'DAN=Det samlede antal varer pÜ lageret er 0, men justeringsplaceringen indeholder et negativt antal, og andre placeringer indeholder et positivt antal.\;ENU=The total quantity of items in the warehouse is 0, but the Adjustment Bin contains a negative quantity and other bins contain a positive quantity.\';
      Text005@1012 : TextConst 'DAN=Vil du stadig slette %1?;ENU=Do you still want to delete this %1?';
      Text006@1011 : TextConst 'DAN=Du kan ikke ëndre %1, fõr lagerbeholdningen i %2 %3 er 0.;ENU=You cannot change the %1 until the inventory stored in %2 %3 is 0.';
      Text007@1013 : TextConst 'DAN=Du skal slette alle justerings-lagerkladdelinjer, fõr du ëndrer %1.;ENU=You have to delete all Adjustment Warehouse Journal Lines first before you can change the %1.';
      Text008@1008 : TextConst 'DAN=%1 skal vëre %2, fordi der findes en eller flere %3er.;ENU=%1 must be %2, because one or more %3 exist.';
      Text009@1014 : TextConst 'DAN=Du kan ikke ëndre %1, fordi der er en eller flere Übne poster pÜ denne lokation.;ENU=You cannot change %1 because there are one or more open ledger entries on this location.';
      Text010@1015 : TextConst 'DAN=Kontrollerer vareposter for Übne poster...;ENU=Checking item ledger entries for open entries...';
      Text011@1016 : TextConst 'DAN=Du kan ikke ëndre %1 til %2, fõr lageret, der er gemt pÜ denne placering, er 0.;ENU=You cannot change the %1 to %2 until the inventory stored in this bin is 0.';
      Text012@1017 : TextConst 'DAN=Vinduet Opsëtning af Online Map skal udfyldes, fõr du kan bruge Online Map.\Se Opsëtning af Online Map i Hjëlp.;ENU=Before you can use Online Map, you must fill in the Online Map Setup window.\See Setting Up Online Map in Help.';
      Text013@1018 : TextConst 'DAN=Du kan ikke slette %1, fordi der er en eller flere finansposter pÜ denne lokation.;ENU=You cannot delete %1 because there are one or more ledger entries on this location.';
      Text014@1019 : TextConst 'DAN=Du kan ikke ëndre %1, da der findes en eller flere %2.;ENU=You cannot change %1 because one or more %2 exist.';
      CannotDeleteLocSKUExistErr@1021 : TextConst '@@@=%1: Field(Code);DAN=Du kan ikke slette %1, fordi en eller flere lagervarer findes pÜ denne lokation.;ENU=You cannot delete %1 because one or more stockkeeping units exist at this location.';
    ...
    VAR
      Text000@1002 : TextConst 'DAN=Leverance,Modtagelse,Pluk,Lëg-pÜ-lager;ENU=Shipment,Receive,Pick,Put-Away';
    BEGIN
      CASE FieldNumber OF
