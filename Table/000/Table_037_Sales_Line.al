OBJECT Table 37 Sales Line
{
  ...
  FIELDS
  {
    ...
    { 5   ;   ;Type                ;Option        ;OnValidate=VAR
                                                   ...
                                                   CaptionML=[DAN=Type;
                                                              ENU=Type];
                                                   OptionCaptionML=[DAN=" ,Finanskonto,Vare,Ressource,Anlëg,Gebyr (vare)";
    ...
    { 57  ;   ;Outstanding Amount  ;Decimal       ;OnValidate=VAR
                                                   ...
                                                   CaptionML=[DAN=Belõb i salgsordre;
    ...
    { 72  ;   ;Purch. Order Line No.;Integer      ;TableRelation=IF (Drop Shipment=CONST(Yes)) "Purchase Line"."Line No." WHERE (Document Type=CONST(Order),
                                                   ...
                                                   AccessByPermission=TableData 110=R;
                                                   CaptionML=[DAN=Kõbsordrelinjenr.;
    ...
    { 94  ;   ;Shipped Not Inv. (LCY) No VAT;Decimal;
                                                   FieldClass=Normal;
                                                   CaptionML=[DAN=Lev. belõb ufakt. (RV);
    ...
    { 107 ;   ;IC Partner Ref. Type;Option        ;OnValidate=BEGIN
                                                   ...
                                                   OptionCaptionML=[DAN=" ,Finanskonto,Vare,,,Gebyr (vare),Varereference,Fëlles varenr.";
    ...
    { 1002;   ;Job Contract Entry No.;Integer     ;OnValidate=VAR
                                                   ...
                                                   CaptionML=[DAN=Lõbenr. for sagskontrakt;
    ...
    { 5714;   ;Special Order Purchase No.;Code20  ;TableRelation=IF (Special Order=CONST(Yes)) "Purchase Header".No. WHERE (Document Type=CONST(Order));
                                                   AccessByPermission=TableData 223=R;
                                                   CaptionML=[DAN=Kõbsordrenr. for specialordre;
    ...
    { 5715;   ;Special Order Purch. Line No.;Integer;
                                                   ...
                                                   AccessByPermission=TableData 223=R;
                                                   CaptionML=[DAN=Kõbslinjenr. for specialordre;
  ...
  {
    VAR
      Text000@1000 : TextConst 'DAN=Du kan ikke slette ordrelinjen, fordi den er knyttet til kõbsordre %1 linje %2.;ENU=You cannot delete the order line because it is associated with purchase order %1 line %2.';
      Text001@1001 : TextConst 'DAN=%1 kan ikke omdõbes.;ENU=You cannot rename a %1.';
      Text002@1002 : TextConst 'DAN=Du kan ikke ëndre %1, fordi ordrelinjen er knyttet til kõbsordre %2 linje %3.;ENU=You cannot change %1 because the order line is associated with purchase order %2 line %3.';
      Text003@1003 : TextConst 'DAN=mÜ ikke vëre mindre end %1;ENU=must not be less than %1';
      Text005@1004 : TextConst 'DAN=Du kan hõjst fakturere %1 enheder.;ENU=You cannot invoice more than %1 units.';
      Text006@1005 : TextConst 'DAN=Du kan hõjst fakturere %1 basisenheder.;ENU=You cannot invoice more than %1 base units.';
      Text007@1006 : TextConst 'DAN=Du kan hõjst levere %1 enheder.;ENU=You cannot ship more than %1 units.';
      Text008@1007 : TextConst 'DAN=Du kan hõjst levere %1 basisenheder.;ENU=You cannot ship more than %1 base units.';
      Text009@1008 : TextConst 'DAN=" skal vëre 0, nÜr %1 er %2";ENU=" must be 0 when %1 is %2"';
      ...
      Text014@1013 : TextConst 'DAN=%1 %2 ligger fõr arbejdsdato %3;ENU=%1 %2 is before work date %3';
      Text016@1040 : TextConst 'DAN="%1 krëves til %2 = %3.";ENU="%1 is required for %2 = %3."';
      ...
      Text026@1025 : TextConst 'DAN=Du kan ikke ëndre %1, hvis varegebyret allerede er bogfõrt.;ENU=You cannot change %1 if the item charge has already been posted.';
      ...
      Text028@1098 : TextConst 'DAN=%1 kan ikke ëndres, nÜr %2 er udfyldt.;ENU=You cannot change the %1 when the %2 has been filled in.';
      Text029@1021 : TextConst 'DAN=skal vëre positiv;ENU=must be positive';
      Text030@1042 : TextConst 'DAN=skal vëre negativ;ENU=must be negative';
      Text031@1093 : TextConst 'DAN=Du skal angive %1 eller %2.;ENU=You must either specify %1 or %2.';
      Text034@1084 : TextConst 'DAN=Vërdien i feltet %1 skal vëre et helt tal for varen i serviceartikelgruppen, hvis feltet %2 i vinduet Serviceartikelgrupper indeholder en markering.;ENU=The value of %1 field must be a whole number for the item included in the service item group if the %2 field in the Service Item Groups window contains a check mark.';
      ...
      Text037@1009 : TextConst 'DAN=Du kan ikke ëndre %1, nÜr %2 er %3, og %4 er positiv.;ENU=You cannot change %1 when %2 is %3 and %4 is positive.';
      Text038@1014 : TextConst 'DAN=Du kan ikke ëndre %1, nÜr %2 er %3, og %4 er negativ.;ENU=You cannot change %1 when %2 is %3 and %4 is negative.';
      ...
      Text042@1055 : TextConst 'DAN=NÜr du bogfõrer udligningsposten, Übnes %1 fõrst;ENU=When posting the Applied to Ledger Entry %1 will be opened first';
      ...
      Text044@1103 : TextConst 'DAN=mÜ ikke vëre mindre end %1;ENU=cannot be less than %1';
      Text045@1104 : TextConst 'DAN=mÜ ikke vëre stõrre end %1;ENU=cannot be more than %1';
      ...
      Text047@1106 : TextConst 'DAN=skal vëre positiv, nÜr %1 ikke er 0.;ENU=must be positive when %1 is not 0.';
      Text048@1108 : TextConst 'DAN=Du kan ikke anvende varesporing pÜ en %1, der er oprettet ud fra en %2.;ENU=You cannot use item tracking on a %1 created from a %2.';
      Text049@1139 : TextConst 'DAN=mÜ ikke vëre %1.;ENU=cannot be %1.';
      Text051@1141 : TextConst 'DAN=%1 kan ikke bruges i en %2.;ENU=You cannot use %1 in a %2.';
      PrePaymentLineAmountEntered@1015 : Boolean;
      Text052@1022 : TextConst 'DAN=Du kan ikke tilfõje en varelinje, da der findes en Üben lagerleverance for Salgshoved, og Afsendelsesadvis er %1.\\Du skal tilfõje varerne som nye linjer i den eksisterende lagerleverance eller ëndre Afsendelsesadvis til Delvis.;ENU=You cannot add an item line because an open warehouse shipment exists for the sales header and Shipping Advice is %1.\\You must add items as new lines to the existing warehouse shipment or change Shipping Advice to Partial.';
      Text053@1017 : TextConst 'DAN=Du har ëndret en eller flere dimensioner pÜ %1, som allerede er afsendt. NÜr du bogfõrer linjen med den ëndrede dimension under finansposter, vil belõb pÜ lagerkonto (mellemregningskto.) ikke stemme, nÜr de rapporteres pr. dimension.\\Vil du beholde den ëndrede dimension?;ENU=You have changed one or more dimensions on the %1, which is already shipped. When you post the line with the changed dimension to General Ledger, amounts on the Inventory Interim account will be out of balance when reported per dimension.\\Do you want to keep the changed dimension?';
      Text054@1023 : TextConst 'DAN=Annulleret.;ENU=Cancelled.';
      Text055@1024 : TextConst '@@@=Quantity Invoiced must not be greater than the sum of Qty. Assigned and Qty. to Assign.;DAN=%1 mÜ ikke vëre stõrre end summen af %2 og %3.;ENU=%1 must not be greater than the sum of %2 and %3.';
      Text056@1011 : TextConst 'DAN=Du kan ikke tilfõje en varelinje, da der findes et Übent lagerpluk for Salgshoved, og fordi Afsendelsesadvis er angivet til %1.\\Du skal fõrst bogfõre eller slette lagerplukket eller ëndre Afsendelsesadvis til Delvis.;ENU=You cannot add an item line because an open inventory pick exists for the Sales Header and because Shipping Advice is %1.\\You must first post or delete the inventory pick or change Shipping Advice to Partial.';
      Text057@1027 : TextConst 'DAN=skal have samme fortegn som salgsleverancen;ENU=must have the same sign as the shipment';
      Text058@1028 : TextConst 'DAN=Det antal, som du forsõger at fakturere, er stõrre end antallet i leverancen %1.;ENU=The quantity that you are trying to invoice is greater than the quantity in shipment %1.';
      ...
      Text060@1041 : TextConst 'DAN=Det antal, som du forsõger at fakturere, er stõrre end antallet pÜ returvarekvitteringen %1.;ENU=The quantity that you are trying to invoice is greater than the quantity in return receipt %1.';
      ...
      AnotherItemWithSameDescrQst@1049 : TextConst '@@@="%1=Item no., %2=item description";DAN=Vi har fundet en vare med beskrivelsen "%2" (nr. %1).\Ville du ëndre det aktuelle varenummer til %1?;ENU=We found an item with the description "%2" (No. %1).\Did you mean to change the current item to %1?';
      SalesLineCompletelyShippedErr@1053 : TextConst 'DAN=Du kan ikke ëndre indkõbskoden for en salgslinje, der allerede er leveret helt.;ENU=You cannot change the purchasing code for a sales line that has been completely shipped.';
      SalesSetupRead@1067 : Boolean;
      LookupRequested@1059 : Boolean;
      DeferralPostDate@1069 : Date;
      FreightLineDescriptionTxt@1033 : TextConst 'DAN=Fragtmëngde;ENU=Freight Amount';
      CannotFindDescErr@1200 : TextConst '@@@="%1 = Type caption %2 = Description";DAN=Kan ikke finde %1 med beskrivelsen %2.\\Sõrg for at bruge den korrekte type.;ENU=Cannot find %1 with Description %2.\\Make sure to use the correct type.';
      ...
      SelectNonstockItemErr@1062 : TextConst 'DAN=Du kan kun vëlge en katalogvare til en tom linje.;ENU=You can only select a nonstock item for an empty line.';
      EstimateLbl@1072 : TextConst 'DAN=Estimat;ENU=Estimate';
      CommentLbl@1046 : TextConst 'DAN=Bemërkning;ENU=Comment';
      LineDiscountPctErr@1073 : TextConst 'DAN=Vërdien i feltet Linjerabat % skal vëre mellem 0 og 100.;ENU=The value in the Line Discount % field must be between 0 and 100.';
