OBJECT Table 1507 Workflow Step Buffer
{
  ...
  PROPERTIES
  {
    ...
    CaptionML=[DAN=Buffer til workflowtrin;
               ENU=Workflow Step Buffer];
  }
  FIELDS
  {
    { 1   ;   ;Order               ;Integer       ;DataClassification=SystemMetadata;
                                                   CaptionML=[DAN=R‘kkef›lge;
                                                              ENU=Order] }
    ...
    { 3   ;   ;Event Description   ;Text250       ;TableRelation="Workflow Event".Description;
                                                   ...
                                                   CaptionML=[DAN=H‘ndelsesbeskrivelse;
    ...
    { 10  ;   ;Parent Event Step ID;Integer       ;TableRelation="Workflow Step".ID WHERE (Workflow Code=FIELD(Workflow Code),
                                                                                           Type=CONST(Event));
                                                   DataClassification=SystemMetadata;
                                                   CaptionML=[DAN=Id for overordnet h‘ndelsestrin;
    ...
    { 16  ;   ;Next Step Description;Text250      ;DataClassification=SystemMetadata;
                                                   CaptionML=[DAN=Beskrivelse af n‘ste trin;
  ...
  {
    VAR
      ThenTextForMultipleResponsesTxt@1002 : TextConst 'DAN=(+) %1;ENU=(+) %1';
      SelectResponseTxt@1000 : TextConst 'DAN=<V‘lg svar>;ENU=<Select Response>';
      EventNotExistErr@1001 : TextConst '@@@="%1 = event description (e.g. The workflow event A general journal batch is does not exist.)";DAN=Workflowh‘ndelsen %1 findes ikke.;ENU=The workflow event %1 does not exist.';
      WhenMissingErr@1003 : TextConst 'DAN=Du skal f›rst v‘lge et N†r-udtryk.;ENU=You must select a When statement first.';
      ResponseNotExistErr@1005 : TextConst '@@@="%1 = response description (e.g. The workflow response Remove record does not exist.)";DAN=Workflowresponsen %1 findes ikke.;ENU=The workflow response %1 does not exist.';
      WhenNextStepDescTxt@1006 : TextConst 'DAN=Derefter, n†r "%1";ENU=Next when "%1"';
      ThenNextStepDescTxt@1007 : TextConst 'DAN=Derefter, s† "%1";ENU=Next then "%1"';
